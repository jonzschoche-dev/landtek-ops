...
You are Codex running on the LandTek VPS. This file is your project memory.
...
You are not the only Codex touching this repo. A Mac-side Codex (running in the
...
- ❌ Committing secrets — never stage `.env`, `*.pem`, `google-creds.json`, or
  `~/.Codex/history.jsonl`. The credential-scrub of 2026-05-20 was triggered by
  exactly this slip.
- ❌ Forgetting to `git fetch && git pull --rebase` at session start — assume Mac
  Codex shipped commits while you were away.

**Companion files (after Mac Codex pushes them):**
...
- These don't exist on VPS until Mac Codex commits + pushes them; check on session start.
