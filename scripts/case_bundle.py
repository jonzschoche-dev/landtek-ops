...
def _gather(mc):
    ...
    docs = docs[:14]                         # keep the bundle reasonable
    ...
    c.close()
    return title, docket, forum, docs, exmap, facts, DT, MN


def _front_matter(path, mc, title, docket, forum, docs, exmap, facts, DT, MN):
...
def _append_doc(out, fp, dpi, quality):
    src = fitz.open(fp)
...
def build(mc, dpi=110, quality=45):
    title, docket, forum, docs, exmap, facts, DT, MN = _gather(mc)
    if not docs:
        sys.exit(f"[bundle] no local supporting documents found for {mc}")
    front = f"/tmp/_front_{mc}.pdf"
    _front_matter(front, mc, title, docket, forum, docs, exmap, facts, DT, MN)
    out = fitz.open()
    out.insert_pdf(fitz.open(front))
    for did, nm, dd, fp, nf, pc in docs:
        _divider(out, f"EXHIBIT {exmap[did]}", f"{nm}" + (f"  ·  {dd}" if dd else ""))
        _append_doc(out, fp, dpi, quality)
