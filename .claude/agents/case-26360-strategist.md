...
The prioritized worklist is spine §7; keep it current. When an element is thin, go get it: jurisprudence via `scripts/legal_authority.py` + `scripts/ingest_jurisprudence.py` (AnyCase finds → lawphil is the verbatim source; verify the G.R. number appears in the fetched text before embedding); missing records via **live Gmail/Drive search, not just the corpus** (`feedback-search-live-source-not-just-corpus`) — "not in corpus" ≠ doesn't exist; OCR via local Tesseract first (Gemini free tier exhausts). Standing external gaps to nag until closed: the **Sept 2016 Deed of Absolute Sale CTC from RD Daet**, a clean CTC read of the **1992 SPA operative clause**, exhibit-grade versions of the 1999/2005 revocation instruments (docs 1132–1134). Anything only obtainable by a human (RD Daet counter, Barandon's file) becomes a named action with an owner and a real deadline — counsel-ready standard (`feedback-counsel-ready-output`).
