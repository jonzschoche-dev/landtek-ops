...
>
> **Ontology version: v0.40 (2026-07-12).** **The comms buildout brought up to law (L4 unblocked).**
...
*Registered here: `meta_pulse_state` (the pulse's own tick-state, 🟢) · `assistant_proposals` (assistant-suggested actions, HITL inbox — A19-pattern, 🟡) · `assistant_nudge_log` (per-receiver nudge ledger — the A71 dose-accounting substrate, 🟡) · `pulse_work_log` (the orchestrator's idempotency ledger, 🟢 — deploy_840) · `leo_shadow_replies` (the headless-Leo shadow-loop reply ledger, 🟡 — test surface, sends nothing). Components: `deadlines.py` (surface/classify/escalate) · `deadline_extractor.py` · `calendar_sync.py` ·
