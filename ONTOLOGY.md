...
>
> **Ontology version: v0.38 (2026-07-11).** **A62 v2 — the survivable record, redesigned and FULLY GREEN.**
...
| A62 | **The record survives the machine.** The SoR (Postgres) is backed up FRESH (≤26h), the copy leaves the box, and a restore has been DRILLED — every other invariant assumes the SoR exists (A50 "rebuild from SoR", A53 "reason from local Postgres"); A62 governs the assumption. A backup whose pipeline can die silently, or that has never been restored, is a hope, not a backup. | 🟢 **asserted — v2 REDESIGN (deploy_862, 2026-07-11), FULLY GREEN.** Operator reassessment killed two architectural flaws: the full dump was **2.6GB of NON-record** (workflow snapshots 2GB · n8n plumbing 0.5GB · dead-sim QA 0.1GB) around a **~86MB actual record**, and the off-box leg died twice on third-party knobs (Drive OAuth scrubbed May; B2 storage-capped). v2: nightly **DOMAIN dump** (`--exclude-table-data` on the bulk; Sunday full dump local-only, 28d) + **the Mac as the off-box node** (`scripts/offbox_backup_pull.sh` via `com.landtek.offbox-backup` launchd, tailnet pull + sha256 verify + a RECEIPT written to the VPS only after a verified copy exists — $0, no third party, different failure domain, A53-clean). `truth_tests/test_survivable_record.py` v2: fresh (40MB floor) · last-run log clean · **off-box receipt <30h + sha matches a local dump** · drill reported — **drill RUN 2026-07-11: restore faithful, documents 2042=2042** (`restore_drill.sh`, peer-built). Negative-tested (no-receipt · stale-receipt bite). B2 retired — no cap raise needed. |
