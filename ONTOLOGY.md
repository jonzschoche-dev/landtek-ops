...
*Registered here: `meta_pulse_state` (the pulse's own tick-state, 🟢) · `assistant_proposals` (assistant-suggested actions, HITL inbox — A19-pattern, 🟡) · `assistant_nudge_log` (per-receiver nudge ledger — the A71 dose-accounting substrate, 🟡). Components: `deadlines.py` (surface/classify/escalate) · `deadline_extractor.py` · `calendar_sync.py` ·
