...
>
> **Ontology version: v0.36 (2026-07-11).** **A76 — relationship equilibrium (the reactive half of A70,
...
| A62 | **The record survives the machine.** The SoR (Postgres) is backed up FRESH (≤26h), the copy leaves the box, and a restore has been DRILLED — every other invariant assumes the SoR exists (A50 "rebuild from SoR", A53 "reason from local Postgres"); A62 governs the assumption. A backup whose pipeline can die silently, or that has never been restored, is a hope, not a backup. | 🟢 **asserted** — `truth_tests/test_survivable_record.py` (deploy-gate + nightly): (a) local dump fresh ≤26h + ≥100 MB sanity floor (a truncated dump "succeeds" silently); (b) backup-log window clean — covers the `rclone`→Drive OFF-BOX copy *without the gate touching the network* (A53-clean); (c) restore-drill days-since REPORTED every run (never recorded yet — the honest gap; record drills in `/root/backups/RESTORE_DRILL.log`). Grounded 2026-07-10: nightly 02:00 `pg_dump` 1.3 GB + rclone offsite + 14d retention existed but was UNGOVERNED (silent-death class). Negative-tested via env override. |
