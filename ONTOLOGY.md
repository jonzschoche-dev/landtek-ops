...
| A9 | A parcel's geometry belongs to exactly one client; a `map_parcels`/`parcels` row may only carry or expose geometry for its own `client_code`. | 🟢 **ENFORCED (block, 2026-07-18)** — extends A5. Triggers `ontvv_v6_map_parcels` + `ontvv_v6_parcels` (fn `ontvv_geometry_isolation`) flipped `log`→`block` per the ALIGNMENT §9 checklist: **the geometry campaign WAS the active-pipeline soak** (88 `parcels` + 70 `map_parcels` real writes 07-12→07-17, 0 true findings — a prior "%V6%" match was matter-code CV6839 noise), pre-flight clean (0 NULL-client rows; 158/158 live rows in scope), rolled-back block-mode exception test RAISED as designed both pre- and post-flip (positive arm verified too: correct-client write passes), **operator approval: Jonathan, 2026-07-18 (chat session)**. **HONEST SCOPE (finding 42435, open):** the trigger fires only when `matter_code` AND `client_code` are both non-NULL — a NULL-matter geometry write is structurally invisible even in block (the V4-null-bypass class; V11-pattern closure directed to this desk). All current live writers populate both columns. |
