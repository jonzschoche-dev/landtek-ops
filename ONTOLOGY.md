...
>
> **Ontology version: v0.24 (2026-07-08).** **Offline sovereignty is now a first-class invariant — A53.** The
...
| A53 | **The stack REASONS with no internet.** The local core — Postgres (SoR) + Ollama inference + embedded `legal_chunks` law + `documents.extracted_text` — is self-contained; **every external service (Gemini · Telegram · Gmail · Drive · GitHub · lawphil) is an EDGE** (delivery / ingestion / binary-view / sync / one-time), **never REQUIRED-TO-REASON.** A document's TEXT must stay local even when its binary is offloaded to Drive (`drive_offload` drops a PDF only when `extracted_text<>''`), so reasoning stays offline-complete. | 🟢 **asserted** — `scripts/offline_audit.py` (deploy_562) verifies the core is offline-capable + classifies every external touchpoint required-vs-edge; **VERDICT green today** (Postgres+Ollama+law+text all local, no external required to reason). The system-level doctrine (`ONTOLOGY_STRUCTURE §5`), now a first-class axiom. Riders: A17 (guard floors), A50 (SoR store-agnostic), A46 (filing local-first). **Watch:** docs with no local text — keep extraction ahead of Drive-offload. |
