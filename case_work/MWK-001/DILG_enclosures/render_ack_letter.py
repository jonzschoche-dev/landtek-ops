...
SRC = f"{MWK}/DILG_MLGOO_ACK_CLARIFICATION_2026-08-26.md"
OUT = f"{MWK}/DILG_MLGOO_ACK_CLARIFICATION_2026-08-26.pdf"
FOLIO = (8.5 * inch, 13.0 * inch)
FOOT = "Zschoche (AIF, Keesey Zschoche) — Acknowledgment and Clarification re Item 3(g)/4(f), 26 Aug 2026"
...
while i < len(lines):
    ...
    if not raw.strip():
        i += 1; continue
    if in_addr:
        if raw.startswith("Dear Mr. Guerrero"):
            in_addr = False
...
doc = SimpleDocTemplate(OUT, pagesize=FOLIO, leftMargin=1.0*inch, rightMargin=1.0*inch,
                        ...
                        title="Acknowledgment and Clarification re Item 3(g)/4(f) — Zschoche to DILG-MLGOO Mercedes, 26 Aug 2026",
