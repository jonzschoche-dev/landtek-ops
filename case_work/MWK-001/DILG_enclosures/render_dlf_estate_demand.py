#!/usr/bin/env python3
"""Render the de la Fuente estate accounting demand (Sep 2026) to 8.5x13 folio PDF."""
import re
...
SRC = f"{MWK}/DEMAND_DLF_ESTATE_ACCOUNTING_2026-09.md"
OUT = f"{MWK}/DEMAND_DLF_ESTATE_ACCOUNTING_2026-09.pdf"
FOLIO = (8.5 * inch, 13.0 * inch)
FOOT = "Zschoche (AIF, Keesey Zschoche) — Demand for Accounting, Estate of Cesar M. de la Fuente, Sep 2026"
...
while i < len(lines):
    ...
    if not raw.strip():
        i += 1; continue
    if in_addr:
        if raw.startswith("Dear Mr. de la Fuente"):
            in_addr = False
        ...
        if raw.startswith("[•] September 2026") or raw.startswith("*(re:"):
    ...
    if raw.lstrip().startswith("|"):
        ...
        if rows:
            w = {3: [3.1*inch, 1.9*inch, 1.5*inch]}.get(len(rows[0]))
...
doc = SimpleDocTemplate(OUT, pagesize=FOLIO, leftMargin=1.0*inch, rightMargin=1.0*inch,
                        ...
                        title="Demand for Accounting — Estate of Cesar M. de la Fuente, Sep 2026",
