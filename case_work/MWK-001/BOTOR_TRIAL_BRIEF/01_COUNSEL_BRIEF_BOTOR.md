# Counsel Brief for Atty. Adan Marcelo B. Botor — Civil Case No. 26-360, trial 9 Sep 2026

> **Internal work product, prepared 2026-08-30 for Jonathan Zschoche to hand to Atty. Botor.**
> Every assertion cites a corpus doc-id (`doc N`) or is flagged. `[OPERATOR-REPORTED]` = stated by
> Jonathan 2026-08-30, not yet documented; verify against the court record. Nothing here has been
> filed or served. Atty. Bonifacio T. Barandon Jr. remains counsel of record in 26-360.

---

## 1. Why you are being briefed

You are counsel of record in the parallel guardianship (Spec. Proc. No. 2680, RTC Br. 41 Daet,
initial hearing held 27 Jul 2026) and are taking over the estate-side responsibilities for the
Keesey heirs. Jonathan intends to consult you on the 26-360 trial because:

1. **The case is the wedge for the whole estate.** 26-360 is one 2,587-sqm parcel, but the legal
   question it decides — the validity of sales made under the 16 Mar 1992 de la Fuente SPA — is the
   common root of at least **seven documented conveyances** out of the estate (§6 below). A ruling
   on the SPA in 26-360 becomes the cornerstone (or the obstacle) for the planned estate-wide RTC
   nullity/reconveyance action. Whoever runs the estate must care how this trial is fought.
2. **A known framing defect in our own filed papers has not been visibly fixed.** The filed SJ
   motion and Jonathan's own Judicial Affidavit lead with a version of the void-sale theory that
   the SPA's own text defeats — and the defense has already loaded the counter (§4). A reframe memo
   for Atty. Barandon has existed since 18 Jul 2026 (`investigations/MWK-DLF/COUNSEL_MEMO_SJ_REFRAME.md`);
   whether it was delivered and acted on is unknown to this desk. **This is the single most
   important thing to raise with lead counsel before 9 September.**
3. **Jonathan takes the stand.** He carries the plaintiff's case. His prep pack is
   `03_TESTIMONY_PREP_SEPT9.md`.

**Coordination caution (raise it, don't paper over it):** Barandon is counsel of record. A consult
is healthy; a second uncoordinated strategy ten days before trial is not. The productive shape is:
Botor reviews this package → one three-way conversation (Jonathan + Barandon + Botor) → agreed
trial frame and division of labor (estate-wide sequencing stays with Botor; the 26-360 courtroom
stays with Barandon unless everyone agrees otherwise).

---

## 2. The case in one paragraph

The Keesey heirs (Patricia Keesey Zschoche, Geraldine Keesey Hoppe, Marcia Ellen Keesey — Patricia
represented by her son Jonathan as attorney-in-fact) own a 2,587-sqm parcel in Purok 4, Brgy. San
Roque, Mercedes, titled **TCT T-52540** (registered to co-owner Geraldine K. Hoppe; docs 272, 233),
tracing to mother title T-4497. The heirs' former agent **Cesar M. de la Fuente** purported to sell
it to **Gloria H. Balane in September 2016** (deed: doc 415, LRA-certified; doc 233 copy) under a
**16 Mar 1992 SPA** (clean LRA CTC: doc 246). That SPA authorized sales **only to holders of
Contracts-to-Sell issued by the prior administrator Ben Llamanzares** (or prior down-payment
holders), after consultation with the principals — a class Balane, by her own Judicial Affidavit,
was never in (doc 1089 T13, T18–19, T21–22: she entered as a ₱50/month *tenant* via Kenneth
Asistin). The SPA had in any event been revoked 15 Aug 2005 (docs 76, 79; published 2020, annotated
2021). The 2016 sale cancelled T-52540 and produced **TCT T-079-2021002126** in Balane's name
(2021; doc 410 recites "T-52540 (TOTALLY CANCELLED)"). The suit: declare the sale void, cancel
…2126, reinstate T-52540, recover possession, accounting + damages.

**Court:** MTC Mercedes, Camarines Norte — Summary Procedure.
**Defendants:** Sps. Efren & Gloria H. Balane; Sps. Jomil & Princess Balane Torralba; Engr. Erwin
Balane. Defense counsel: Atty. Ronald A. Ramos.

---

## 3. Procedural posture (verified spine + the operator-reported update)

| Date | Event | Source |
|---|---|---|
| Feb 2026 | Complaint filed | docs 424/425 |
| 6 Feb 2026 | Court Order | doc 423 |
| 7 Apr 2026 | Defendants' Answer w/ affirmative defenses + counterclaim | doc 405 |
| 13 Apr 2026 | Plaintiff's Reply | docs 398–403, 771 |
| 24 Apr 2026 | **Plaintiff's Motion for Summary Judgment** (+ Annex A affidavit) | docs 393, 394 |
| 13 May 2026 | Pre-trial held; **Gloria Balane's Judicial Affidavit EXCLUDED** (she is in Canada; only a scanned consularized copy offered) | doc 1088 recitals |
| 19 May 2026 | Defendants' Manifestation: tender of excluded evidence + motion to admit the affidavit | doc 1088 (affidavit itself = doc 1089) |
| ~1 Jun 2026 | Our Comment/Opposition to re-admission | doc 1087 |
| ~2 Jun 2026 | Mediation held | spine §2c |
| 27 Jul 2026 | Guardianship initial hearing, RTC Br. 41 Daet (your matter) | MASTER_PLAN §1; outcome not yet in corpus |
| **28 Aug 2026** | **MTC ORDER (Judge Lourdes Clarissa Donnatilla K. Cu): mediation/JDR failed; trial calendar set.** Plaintiff's witnesses: **9 Sep · 7 Oct · 14 Oct · 4 Nov · 11 Nov 2026**; defendants' witnesses: **2 Dec · 9 Dec 2026 · 13 Jan · 3 Feb · 10 Feb 2027** — all 1:30 PM. Counsel had **3 days to manifest non-availability**, else dates final and non-transferable. Order is **silent** on the SJ and JA-admission motions. | **VERIFIED** — scan + transcription: `case_work/MWK-001/court_orders/ORDER_2026-08-28_trial_setting.md` |
| **9 Sep 2026, 1:30 PM** | **TRIAL DAY 1 — Jonathan testifies as Patricia's witness** | VERIFIED (28-Aug Order) |

**⏰ IMMEDIATE (expires ~31 Aug / 1 Sep):** confirm TODAY with Atty. Barandon whether he has any
conflict on any of the ten dates and whether a non-availability manifestation was filed within the
3-day window. After it lapses the calendar is final and non-transferable — including the December
dates the defense needs for its Gloria-in-Canada problem.

**Trial-shape read-offs from the calendar:** plaintiff has **five** hearing dates — enough for
Jonathan's direct + cross plus supporting witnesses and the formal offer; counsel should now plan
the witness order and which documentary exhibits (including the pattern bundle, §6) land on which
date. The defense's first date is **2 Dec** — their excluded-JA / remote-testimony problem for
Gloria must surface as motion practice before then; watch for it and oppose on the record already
built (doc 1087).

**Unknowns still to close with Barandon's file before 9 Sep:**
1. Has the court ruled on the **SJ motion**? The 28-Aug trial-setting order is **silent** on it
   (see §3a).
2. Has the court ruled on **admission of Balane's Judicial Affidavit**? Also silent in the 28-Aug
   order. If exclusion stands, the defense has no principal direct testimony — and their first
   presentation date (2 Dec) forces the issue into motion practice this fall.
3. ~~What is set for 9 Sep~~ **ANSWERED by the 28-Aug Order:** plaintiff's presentation of
   witnesses, five dates, 1:30 PM.
4. What did the pre-trial order stipulate/mark? (Stipulations bind the trial; we have never read
   the pre-trial order itself.)

### 3a. The silent Summary-Judgment motion — Jonathan's flag (2026-08-30), and the counsel moves

Jonathan finds it strange that no written resolution of the SJ motion has issued. He is right to:
the motion was filed **24 Apr 2026** (doc 393) and, on what the corpus shows, was submitted for
resolution by roughly May–June. Lower courts must resolve pending matters within **three months of
submission** (Const. Art. VIII §15(1)); on that clock a written resolution was due around
July–August. Three explanations, in descending likelihood — each with its own move:

1. **A separate order exists that we have not seen** (served on counsel only, or in the mail). The
   28-Aug trial-setting order — now in hand — does **not** resolve the SJ, so if a resolution
   exists it is a separate issuance. → *Move: a records check at the MTC docket — certified copies
   of every order issued since April. Cheap, fast, and it also captures the pre-trial order and the
   JA-admission ruling. Do this first; nothing else in this section matters until it's done.*
2. **The court is deferring to trial** — treating the SPA's interpretation as a triable issue
   (exactly the defense's manufactured-issue play, dossier §"Weaknesses"). A trial setting with the
   motion unresolved functions as a denial without saying so. → *Move: proceed fully trial-ready
   (this package's posture anyway), and note that under Rule 35 as amended (2019) the court must
   state its reasons on the record when granting or denying — a one-line or silent denial is a
   preserved point, for counsel's judgment whether to press it.*
3. **The motion is genuinely unresolved and forgotten.** → *Move: a respectful Motion for
   Resolution (or manifestation) reciting the filing and submission dates. Filed before trial, it
   either surfaces the ruling or puts the delay on record; either way we stop flying blind.*

**What NOT to do:** do not treat the silence as leverage for a certiorari detour or an
inhibition fight on the eve of trial — that trades a strong record for a procedural war. The
silence is a records problem first, a preserved-error point second, and a grievance never.
And whatever the SJ's fate, **the JA-admission ruling matters more for 9 Sep**: whether Gloria's
affidavit is in or out defines the entire cross plan.

---

## 4. Theory of the case — and the crack lead counsel must fix

### 4a. The winning frame (lead with this)

**The 1992 SPA is a special power of limited object.** Its operative clause (doc 246, read clean;
quoted verbatim in `04_CAMPAIGN_PATTERN_MEMO.md` §2) authorizes de la Fuente to sell **only** "in
favor of third persons in possession of CONTRACTS TO SELL executed by the previous administrator,
MR. BEN LLAMANZARES," and/or prior down-payment holders, **after prior consultation with** the
principals. Three limits sit on the face of the instrument: **object** (the closed buyer class),
**purpose** (proceeds to pay the estate's unpaid realty taxes), **procedure** (prior consultation).

**Balane is outside the class — proven by the defense's own papers:**
- Her JA: she entered by buying "rights and possession" from Kenneth Asistin (₱16,500) and paid
  **₱50/month rent** to the owners' side; she holds **no Llamanzares Contract-to-Sell** (doc 1089
  T13, T18–19, T21–22).
- Their own receipts exhibit (doc 414): **earliest receipt is 20 Oct 2003** — a decade after the
  1992 SPA; none names Llamanzares; none predates the SPA. So she was not a "down-payment holder
  within the SPA's meaning" either. (This also converts SJ ¶10 from assertion to record.)
- She concedes she **never communicated with the owners** before the sale (doc 1089 T43).

A sale outside a special agent's mandate is **void as to that buyer**; good faith cannot cure it
(*nemo dat quod non habet*; Civil Code Arts. 1874, 1877–1878; special powers strictly construed —
*Cosmic Lumber*, G.R. 114311; *Bautista-Spille v. Nicorp*, G.R. 214057). **This bypasses the
good-faith fight and moots the revocation-notice dispute.**

**Backup prong (argue second, with honest limits):** the SPA was revoked 15 Aug 2005 (docs 76, 79)
— 11 years before the sale — and a buyer dealing with an agent bears the duty of inquiry
(*Yoshizaki*, G.R. 174978); Balane admits she only "inspected the Torrens title" and never asked
the owners. Honest limits (do not overstate): the 2005 disowning clause does not name Cesar; no
document proves 2005 notice *to him* (Salvador denies it, doc 407 S32); publication came 2020,
annotation 2021. Under the lead frame none of that matters.

### 4b. The "negotiate" argument — not retired, LAYERED (revised 2026-08-30 per Jonathan's flag)

**The internal history, honestly:** the filed SJ motion argues the SPA "does not contain any
express grant of authority to sell … but only to 'negotiate'" (doc 393 ¶5–6); Jonathan's JA swears
the same (doc 441 Q53). A July war-room audit flagged this as dangerous *as a sole lead*, because
the SPA also empowers the agent "to sign and execute the necessary documents … as well as to
receive the proceeds of the aforementioned sale" — text our own witness reads into the record
(doc 441 Q23) — and the defense has loaded exactly that counter (Answer ¶39: the SPA "read as a
whole" allowed a sale). **Jonathan's response (2026-08-30) is doctrinally right: the
strict-construction jurisprudence keeps the negotiate argument alive.** The correct posture is
neither "lead only with negotiate-only" nor "retire it" — it is **argument in the alternative,
each prong independently voiding the sale:**

**Prong 1 — No power to sell at all (strict construction).** The SPA's operative grant is "to
negotiate for the sale." **Nowhere in the instrument do the dispositive words appear — sell,
convey, transfer, alienate, dispose, hypothecate, encumber.** The law requires them:
- Art. 1874 — sale of land through an agent requires *written* authority, else void;
- Art. 1878(5) — an SPA is required for any contract transmitting ownership of an immovable;
- **Art. 1879 — "A special power to sell excludes the power to mortgage; and a special power to
  mortgage does not include the power to sell"** — the Code's own declaration that dispositive
  powers are never implied one from another; a fortiori, none is implied from "negotiate";
- *Cosmic Lumber v. CA* (G.R. 114311, 1996) — sale authority must appear in "clear and
  unmistakable language"; *Woodchild Holdings v. Roxas Electric* (G.R. 140667, 2004) — powers of
  attorney are strictly construed and never extended by implication; *Bautista-Spille v. Nicorp*
  (G.R. 214057, 2015; **in the firm's verified law library**) — general/administrative authority
  or mere authority to negotiate does not include the power to sell, and the agent's sale is void;
  *MCIAA v. Unchuan* (G.R. 182537, 2016; in library) — void for want of written sale authority,
  and the nullity action never prescribes. The negotiate-line cases (*AF Realty v. Dieselman*;
  *Litonjua v. Eternit*) hold authority to negotiate/find a buyer is not authority to conclude the
  sale — **counsel to verify these two citations before briefing** (not yet in the verified
  library).
- The defense's "sign and execute … receive the proceeds" clause is answerable on strict
  construction: it is **ancillary machinery for "the aforementioned sale"** — the class-limited,
  consultation-gated sale the instrument describes — not an independent, at-large dispositive
  grant. Under *Woodchild*, ancillary words cannot extend the power by implication.

**Prong 2 — Even assuming a sale power, Balane is outside it.** The trap in the defense's own
argument: **"read as a whole" is self-defeating.** If reading the instrument as a whole yields a
sale power, the same whole reading carries, in the same sentence, the Llamanzares-CTS class limit
and the prior-consultation condition. They cannot keep the empowering clause and drop the limiting
one. Balane concedes she held no Llamanzares contract and never contacted the owners (doc 1089
T13/T18–19/T43; receipts doc 414). A sale beyond the SPA's limits is void — *Tumibay v. Lopez*
(G.R. 171692, 2013; in library: agent exceeding the SPA's limits, buyer chargeable with knowledge
→ void, reconveyance). This prong is proven by the defendants' own exhibits and is immune to ¶39.

**Prong 3 — In any event, revoked 2005** + the buyer's duty of inquiry (*Yoshizaki*; *Sarili v.
Lagrosa*, *Bucton v. Go*, *Manalese v. Ferreras* — all in library on buyer-through-agent
diligence), with the honest limits already noted (§4a).

**The division of labor that resolves the July-vs-now tension:** *counsel argues all three prongs
in the alternative on paper* — that is what lawyers do, and each prong is independently
dispositive. **Jonathan the witness argues none of them** — a witness cannot plead in the
alternative; on cross he concedes the clause exists and pivots to the class limitation, exactly as
his prep pack drills. The July guidance protects the witness; Jonathan's point restores the full
arsenal to the briefs. Both are right, at different altitudes.

**What lead counsel must still do:** align the trial memorandum / formal offer to this layered
frame; handle the JA Q53 wording and the known typos (§4c) before the defense uses our own
affidavit against us. **This is the concrete substance behind Jonathan's "not proactive enough"
concern — raise it directly and get an answer on how the trial will be framed.**

### 4c. Also banked in our papers (clean up before the stand)

- Doc 441 Statement of Purposes (g)/(h) says the sale was "September 26, **2019**" — wrong; Q35 of
  the same affidavit and the deed say Sept 2016. Errata or on-record correction with counsel.
- Area discrepancy: Complaint ¶4 says **28,891** sqm (T-52540's total-parent figure); JA Q11–12
  says 28,981. Use the title's figure.
- Deed-date drift: doc 415 executed 24 Sep 2016; the doc 233 copy recites 29 Sep 2016; the Deed of
  Confirmation (doc 240) fixes the operative registered deed as the 29-Sep/Asis notarization.
  Reconcile execution vs notarization once, and answer it calmly if probed.
- Avoid "misappropriated" (JA Q25) — say "never remitted"; non-remittance is an Art. 1891
  accounting claim, not a voiding element (the deed recites the agent received ₱250,000; Salvador
  swears it went to taxes, doc 407 S24 — contested).

---

## 5. The defense case and its condition

The defense's entire case is Gloria Balane's good faith, carried by her Judicial Affidavit —
**which was excluded at pre-trial** and is the subject of their pending re-admission motion
(doc 1088). If exclusion stands, they have no principal direct testimony. Their remaining
witnesses: Salvador Osum dela Fuente (doc 407), Princess Balane Torralba (doc 408), Engr. Erwin
Balane (doc 409) — all in corpus, all read.

Her affidavit, if admitted, is **double-edged for them**: it hands us the tenancy admission
(T18–19), the no-contact admission (T43), the reliance-on-verbal-say-so (T30 vs T56), her
public-office status 1997–2017 (T59 — higher diligence standard), and her naming of other
de la Fuente buyers (T39–T42) — the door to the campaign pattern (§6). The cross map is in the
spine §2d and the testimony pack.

Their equities story (occupancy since 1975, taxes, improvements) goes to damages/improvements
equities, not to title: a void deed is imprescriptible (Arts. 1409–1410; *Heirs of Tulauan*), and
laches cannot validate it. The human narrative must still be answered on the stand — owners abroad,
no annotation ever gave them notice, they published upon learning (2020), annotated (2021), sued.

**Defense procedural watch-item:** Answer ¶38 seeks joinder of Cesar's estate as indispensable
party. The opposition frame: reconveyance runs against the current titleholder; the estate is at
most a necessary party for the accounting. Confirm with Barandon this opposition is ready.

---

## 6. The greater picture — the documented de la Fuente campaign

This is the context Jonathan fears will be lost at trial. **It is documentary, not theory** — but
it must be used with discipline (full treatment: `04_CAMPAIGN_PATTERN_MEMO.md`).

**The campaign's full scale is 20+ named transferees and roughly 35–40 derivative titles carved
out of mother title T-4497** (41 transfer events in the title map; Complaint and transferee
matrix). Within that, **seven de la Fuente sales are read and quotable in corpus** — the deeds
themselves, each signed by him "as Attorney-in-Fact of the Heirs of Mary W. Keesey": Balane (docs 233/239/366/415/432 — five versions
of one deed), **Rosalina Hansol — Gloria Balane's sister** (doc 413), Delia Nacasa, Alberto Victa,
Dolores Vela, Jose Pascual Jr., and a probable Cesar Ramirez (all in the RD-certified multi-deed
bundle **doc 590**). Balane's own JA corroborates the campaign by naming Ramirez and her sister as
fellow buyers (doc 1089 T39–T42).

**The two-instrument kill-shot (upgraded 2026-08-30 — Jonathan's flag, confirmed against
`T4497_AUTHORITY_OCR_FINDINGS.md`):** de la Fuente cited two different authority instruments across
the campaign, and **neither authorized the sale made under it**:
- **1991 Agcaoili SPA** (Doc 260/Pg 53/Ser 1991, 22 Aug 1991) — **administration only: no clause
  selling, conveying, or mortgaging anything** (vision-OCR verified from the instrument itself,
  docs 72 [cleanest, with notarial page] / 82 / 35; CTC in doc 369 pp 1–4). **The Hansol deed
  (doc 413) rests on this instrument** — a sale of land by an agent with no written authority to
  sell is **void under Art. 1874**, full stop; no class analysis even needed. *(Court-use gate:
  the reading is OCR-grade — have counsel eyeball the instrument image before offering; then it is
  exhibit-ready.)*
- **1992 Munda SPA** (doc 246) — a sale power, but class-limited to Llamanzares CTS /
  down-payment holders (§4a). The Balane and doc-590 deeds rest on this one.

And the deed proving the 1991-SPA void sale — to Gloria's own sister — was **procured from the RD
by defendant Erwin Balane himself** (doc 413, Ref. 2026002049). Their own exhibit documents a void
family purchase; handled correctly at trial, it backfires on them entirely.

**The verified fraud signals across the campaign:**
- **Below-value pricing:** Balane paid ₱96.6/sqm against the estate's own assessor market value of
  ~₱2.5M for the same parcel (~10% of MV; ledger docs 238/411); the other proven sales run
  ₱43–₱150/sqm.
- **Purpose betrayed:** the SPA's stated purpose was to raise money for the estate's unpaid realty
  taxes — the estate still owed **₱775,202** in RPT in 2023 (doc 290). Where the ~₱458,400 of
  known proceeds went is unaccounted (Art. 1891 accounting claim against Cesar's estate).
- **Serial re-execution:** the same Balane sale was papered **five times across 2016** with
  varying dates/notaries — evidence of a defective deed being patched, not a clean transaction.
- **Void sub-delegation:** Cesar purported to delegate his agency to Salvador O. dela Fuente in
  2012 (doc 38); absent principal authorization, sub-agency is void (Art. 1892). Cesar **died
  21 Jun 2017** (doc 673, RTC record in CV 6839) — so the 2021 registration cascade necessarily
  ran through others, four years after the agent's death.

**What "syndicate" is and is not, on the record:** a coordinated *campaign by one fiduciary*, with
family links among buyers and an unauthorized successor, is VERIFIED. A criminal syndicate
involving officials is **INFERENCE — PENDING VERIFICATION** and must never be asserted as fact in
court. The strongest verified thread in that direction: **the name "Romel C. Jacinto" appears as
instrumental witness on the Balane deed and the 2017 Deed of Confirmation AND as the OIC Register
of Deeds who issued Balane's title on 23 Nov 2021** (document faces read: doc 369 pp 10/16/22, per
`T4497_AUTHORITY_OCR_FINDINGS.md`). The *names on the documents* are verified; that it is the
*same person*, and any inference of complicity, is not — counsel decides whether to probe it on
cross, never to assert it. The ARTA dockets against Mercedes officials and the NBI referral remain
separate, human-gated tracks.

**Strategic sequencing (the estate-wide stakes of this trial):**
- The estate-wide consolidated RTC nullity/reconveyance action
  (`investigations/MWK-DLF/RTC_COMPLAINT_OUTLINE.md`) is deliberately **staged behind 26-360** —
  a favorable ruling on the 1992 SPA becomes its anchor; an adverse one arms every other holder.
- Inside 26-360, the campaign evidence (docs 590, 413 + Balane's T39–T42) is legitimately usable
  to defeat good faith and corroborate the class-limitation reading — **if counsel formally offers
  it.** Whether Barandon intends to is exactly the question to put to him.
- Most campaign sales predate the 2005 revocation, so the estate-wide theory rides the **object +
  purpose limits (V1)**, not revocation timing — Balane's post-revocation sale is the outlier bonus.

---

## 7. What we ask of you (concrete)

1. **Before 9 Sep, join one strategy conversation with Jonathan and Atty. Barandon** and get
   closure on: (a) the SJ / affidavit-admission rulings and the pre-trial order; (b) the
   class-limitation reframe as the trial frame; (c) whether the doc 590/413 pattern bundle will be
   formally offered; (d) the JA errata; (e) the indispensable-party opposition.
2. **Guardianship ↔ trial coordination:** the guardianship record and 26-360 share evidentiary
   foundations (the SPAs, revocations, non-partition). Confirm nothing filed in Spec. Proc. 2680
   (including any amended ground per `GUARDIANSHIP_AMENDMENT_PACKAGE.md`) contradicts the 26-360
   record — the defense will read both files.
3. **Estate-wide sequencing ownership:** as estate counsel, take ownership of the staged RTC
   end-vehicle and the four unread root deeds (Valledor, Santiago, Tychingco, Vergara — RD pulls),
   so the 26-360 outcome lands into a prepared next move.
4. **Witness prep session with Jonathan** on `03_TESTIMONY_PREP_SEPT9.md` — ideally with Barandon;
   the concede-and-pivot must be rehearsed aloud, not read.
5. **Evidence hardening you can commission now:** Cesar's PSA death certificate (doc 673 is the
   court-record substitute); the CV 6839 order showing Cesar replaced as the heirs' representative
   in 1999; fresh CTCs only if the judge signals a genealogy fight (the certified chain in corpus
   is presentable — spine §3).

---

*Prepared from: BALANE_EVIDENCE_SPINE.md (v2 + 2026-07-18 audit), DOSSIER_MWK-DLF-VOID.md,
COUNSEL_MEMO_SJ_REFRAME.md, T4497_DELAFUENTE_ROOT_THEORY.md, MASTER_CASE_FILE_CV26360_FINAL.md.
Provenance discipline per MASTER_PLAN §4B. MWK-001 only.*
